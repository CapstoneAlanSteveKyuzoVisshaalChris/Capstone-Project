...
def assistant(inputValue, storage):
    ...
    if inputValue == "QUICK":
        genlen = len(storage.getLikesGenre())
        if genlen > 0:
            specgen = random.randint(0, genlen-1)
            genre = storage.getLikesGenre()[specgen]
        else:
            genre = "Action"
        if len(storage.getHistory()) > 0:
            siz = 0
            if len(storage.getHistory()) > 3:
                siz = 3
            else:
                siz = len(storage.getHistory())

            keyspec = random.randint(0, siz-1)
            movid = test.searchMovieName(storage.getHistory()[keyspec])["results"][0]["id"]
            print(movid)
            keylist = requests.get('https://api.themoviedb.org/3/movie/' + str(movid) + '/keywords?api_key=' + str(test.key)).json()["keywords"]
            keysiz = len(keylist)
            keyspec = keylist[random.randint(0, keysiz-1)]
            keywords = [keyspec][0]["name"]
            keywords = [keywords]
        else:
            keywords = ["robot"]
        time = ""
        ...
        if len(movieList) != 0:
            ...
            return[[poster, "'<b>" + title + "</b>' ~~~~~ Here is an overview: " + overview,"Are you looking for a movie recommendation, trying to update your movie preferences, or trying to learn more about Recommend-Man?"],statelist.startState()]
